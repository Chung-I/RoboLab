...
"""Evaluate the Pi0 family of policy backends on the mass/CoM variation study.

Registers only the 10 study envs (2 tasks x 5 mass/CoM conditions; spec
§3.1) via ``auto_register_droid_envs_mass_variations``, instead of the full
task suite registered by ``policies/pi0_family/run.py``. Covers pi0,
pi0_fast, pi05, paligemma, and paligemma_fast — all served by
