...
- **Inputs (produced by Plan 2, schemas as-built and verified by the Plan-2 final review):** `output/probe_dataset/pi05.npz` — `acts` (N=2010, L=18, P=3, D=2048) f16; labels `mass_kg, com_offset_m, com_axis_idx, wrench (N,6), contact_force_norm, joint_pos (N,7), drift, precontact_mask, in_window_mask, episode_id, step, object_id, steps_since_anchor`; per-condition `output/replay_corpus/<obj>/<cond>/ft.npz` (keys incl. `actions`, `object_root_pose`, `anchor_step`, `matched_window_N`); capture: `output/activations/pi05/<obj>/<cond>/acts.npz` + one shared `output/activations/pi05/meta.json`; local checkpoint `~/.cache/openpi/pytorch/pi05_droid_jointpos/` (parity-passed; **load it under the local config name `pi05_droid_jointpos_polaris`** — see convert_pi05.md for the equivalence note).
- **Activation access rule (binding):** never slice `acts[:, l, p, :]` raw. Positions carry `stream`/`valid_dims` in the capture meta (P0/P1 = PaliGemma, dims [0,2048); P2 = action-expert stream, dims [0,1024) with zero padding above — dim k at P2 is a *different network's* dim k). Use `analysis/mass_com/acts_io.load_acts` semantics: per-position arrays sliced to `valid_dims`; never compare or concatenate the same dim index across P2 and P0/P1.
- **f16-clip rule (pre-registered):** all 53,944 clipped values sit at layer 17 / position 0 (dim 1577 saturated on 100% of steps). For any probe or patch at (PG17, position 0), exclude dims whose aggregate clip count in `meta.json f16_clip` exceeds 1% of steps from the feature set, and say so in the results doc. All other (layer, position) cells are clip-free.
- **Scrub data caveat (binding, from the design doc addendum):** scrub episodes' pre-grasp segment is bit-identical across conditions (states-mode re-derivation); whole-episode `max_drift` and `matched_window_N` are NOT per-condition physics signals for scrub (windows are identically 25). Use post-`precontact_boundary` windows for any drift-based statistic; the per-step `drift` label exists for exactly this. `build_pairs`' use of `min(window_a, window_b)` remains valid — it is a conservative alignment bound, not a physics readout.
- **Carton `late` mask is empty** (in_window runs to the end of every 157-step carton episode): late-phase probe cells contain only scrub rows, and the carton↔scrub transfer number is undefined on the `late` mask — report those cells as N/A, not zero.
...
3. The Task-3 probe grid gains `carry` cells for all 18 grid targets (object_id cells remain precontact-only by design; same machinery, same guards); the mass-null claim in T6 is stated on `carry` (where the certificate has its only chance of passing) with `window` reported alongside.
