...
"""One-object test-lift task: measuring cup on the table (study docs/studies/2026-09-09-test-lift-v3-plan.md, Task 3).

Scene: `assets/scenes/mugs4_measuringcup_drill_bowl.usda`, chosen because it carries a prim
literally named `measuring_cup` (checked with
...
`ceramic_mug`, `red_mug`, `cordless_drill`, `sideways_white_mug`, `upright_white_mug` -- are
...
`analysis/test_lift/batch.py:neighbour_distances` only sees declared bodies.
    ...
    body                    (x, y)                  planar dist to measuring_cup
    red_mug                ( 0.3919,  0.0378)      0.225 m
    sideways_white_mug     ( 0.5486, -0.0485)      0.223 m
    cordless_drill         ( 0.6765,  0.0329)      0.374 m
    upright_white_mug      ( 0.3096,  0.2240)      0.416 m
    ceramic_mug            ( 0.5833,  0.2212)      0.458 m
    bowl                   ( 0.3578,  0.3764)      0.563 m

No neighbour sits within 10 cm of `measuring_cup` (closest is `sideways_white_mug` at
22.3 cm), so no repositioning is applied (cube precedent, moved-only-if-crowded).
"""
import os
...
@configclass
class MeasuringCupTestLiftScene:
    scene = AssetBaseCfg(
...
@dataclass
class MeasuringCupTestLiftTask(Task):
    scene = MeasuringCupTestLiftScene
    terminations = TestLiftTerminations
    contact_object_list = ["measuring_cup", "bowl", "ceramic_mug", "red_mug", "cordless_drill",
                           "sideways_white_mug", "upright_white_mug"]
    instruction: str = "Test-lift the measuring cup"
